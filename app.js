...
async function cleanDirectory(path, options, deletedFiles = []) {
  ...
  for (let i = 0; i < files.length; i += 1) {
    if ((!filter) || (filter && filter(files[i]))) {
      ...
      if (stats.isDirectory()) {
        await cleanDirectory(filepath, options, deletedFiles);
...
module.exports = function (path, options, callback) {
  ...
  if (options && typeof options !== 'object') { throw Error(`${moduleName} error: second param must be an object`); }
